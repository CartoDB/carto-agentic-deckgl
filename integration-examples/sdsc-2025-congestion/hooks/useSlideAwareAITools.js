...
/**
 * Parse tool response from WebSocket
 */
...
/**
 * useSlideAwareAITools - Hook for AI-powered map tools with slide awareness
 */
export function useSlideAwareAITools({
  wsUrl,
  appState,
  slidesConfig,
  demoId,
  ...
  }, [slidesConfig, hasAppState]); // Recreate when slidesConfig changes OR appState becomes available
  ...
  }, [slidesConfig, demoId]); // Remove appState dependency - use ref instead
  ...
  const handleStreamChunk = useCallback((data) => {
    ...
    if (data.isComplete && !data.content) {
      ...
      // Don't show "executing" loader here - only show when tool_call arrives
      setLoaderState(null);
      return;
    }
  ...
  const handleToolCall = useCallback(
    (response) => {
      ...
      if (error) {
        ...
        // Add a warning message to the chat explaining the validation error
        addMessage({
          type: 'action',
          content: `⚠️ Tool "${toolName}" failed: ${error.code === 'VALIDATION_ERROR' ? 'Invalid parameters sent by AI. Please try rephrasing your request.' : error.message}`,
        });
      ...
      if (executor && data) {
        ...
        // Unknown tool called by AI - provide helpful error message
        ...
        // Add a message to the chat explaining the issue
        addMessage({
          type: 'action',
          content: `⚠️ The AI tried to use a tool that doesn't exist: "${toolName}". Please try rephrasing your request using available features like: update layer styles, reset visualization, navigate slides, change view, etc.`,
        });
