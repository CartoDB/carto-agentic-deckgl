...
server.listen(PORT, () => {
  ...
  console.log(`🤖 Google ADK Model: ${process.env.GOOGLE_ADK_MODEL || 'carto::gemini-2.5-flash'}`);
  console.log(`📡 WebSocket endpoint: ws://localhost:${PORT}/ws`);
  console.log(`🔗 OpenAI Responses endpoint: http://localhost:${PORT}/api/openai-chat`);
  console.log(`🔗 Google ADK endpoint: http://localhost:${PORT}/api/googleadk-chat`);
