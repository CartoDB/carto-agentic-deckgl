...
import { getMCPStatus } from './agent/mcp-tools.js';
import { getConfiguredProvider } from './agent/providers.js';
  ...
  ws.on('message', async (data) => {
    try {
      ...
      if (rawMessage.type === 'chat_message') {
        ...
        console.log(
          `[WS] Tool result received: ${toolResult.toolName} - ${toolResult.success ? 'success' : 'failed'}`
        );
